...
- **Target market — RESOLVED (refined 2026-06-10):** English, single-MCQ format, **audience-first** (monetization deferred to a later Telegram funnel phase — correct given tiny Indian Shorts RPM). Concentration narrowed from "broad daily GK" to the **SSC / Banking / Railways "General Awareness" tier** (SSC CGL/CHSL, IBPS/SBI, RRB) — sharper *intent signal* (the win metric above) and an exact format fit, since those exams' GA section is itself static-GK + current-affairs MCQs. UPSC excluded (analytical/long-form, poor quiz-short fit). See `docs/specs/2026-06-10-gk-topic-selection-design.md`.
- **Channel count — RESOLVED 2026-06-10: ONE combined GA channel, not per-exam channels.** SSC / Banking / Railways GA syllabi overlap ~80% (shared static GK + the same current affairs), so one daily question serves all three pools at once (tag exam relevance in the caption/answer line). Three near-clone exam channels would (a) trigger YouTube's "similar template across multiple channels" inauthentic-content purge — a ban risk, not growth; (b) multiply the human approval-gate labor (already 3 platforms/video) against the minimal-labor thesis; and (c) splinter one audience into three weak signals instead of a real independent read. "Spray to discover" means different *themes*, not splitting one niche. **Per-exam split is deferred behind a data trigger:** revisit only after this channel shows intent signal AND an exam's syllabus diverges enough to need distinct content AND the pipeline is automated enough that a second channel's marginal labor is ~0. A second portfolio bet should be a *different theme*, not a second exam.
- **Biggest gating risk — RESOLVED 2026-06-08 (GO):** Meta App Review + Business Verification are NOT required for self-posting to the operator's own FB/IG accounts — role-holder-only apps stay in Standard Access. Precondition: add own FB Page + IG Business/Creator accounts as role-holders on the Meta app. See `docs/posting-pipeline-api-research.md`. Publish access for YT/FB/IG is now cleared.
