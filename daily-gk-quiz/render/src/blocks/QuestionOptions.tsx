...
export const QuestionView: React.FC<{ question: string; tone?: "light" | "dark" }> = ({ question, tone }) => (
  <h1 style={{ fontSize: 74, fontWeight: 900, lineHeight: 1.08, letterSpacing: "-.02em",
               color: tone === "dark" ? "#171410" : STANDARD.text, margin: 0 }}>
    {question}
  </h1>
);

export const OptionsView: React.FC<{ options: Option[]; correctLetter: string; revealed: boolean; tone?: "light" | "dark" }> =
  ({ options, correctLetter, revealed, tone }) => (
        ...
        return (
          <div key={o.letter} data-testid={`opt-${o.letter}`} data-correct={String(isCorrect)}
               style={{ display: "flex", alignItems: "center", gap: 26, padding: "28px 34px",
                        ...
                        color: tone === "dark" ? "#171410" : STANDARD.text }}>
